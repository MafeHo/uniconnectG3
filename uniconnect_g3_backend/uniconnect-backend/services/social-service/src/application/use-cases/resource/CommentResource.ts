...
import { RecursoConComentarios } from '../../../domain/decorators/RecursoConComentarios';
...
export class CommentResource {
  ...
  async execute(
    ...
    const recursoDecorado = new RecursoConComentarios(baseRecurso, comment);
    const metadata = recursoDecorado.getMetadata();

    // 4. Guardar cambios en el campo comments
    ...
    return {
      success: true,
      comment,
      totalComments: (metadata.comments as any[]).length,
    };
  }
}
